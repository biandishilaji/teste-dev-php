...
*/
(function (d) { typeof define === 'function' && define.amd ? define(['jquery', 'datatables.net'], function (n) { return d(n, window, document) }) : typeof exports === 'object' ? module.exports = function (n, o) { n || (n = window); if (!o || !o.fn.dataTable)o = require('datatables.net')(n, o).$; return d(o, n, n.document) } : d(jQuery, window, document) })(function (d, n, o, m) {
  var i = d.fn.dataTable, x = 0, y = 0, k = i.ext.buttons, l = function (a, b) {
    typeof b === 'undefined' && (b = {}); !0 === b && (b = {}); d.isArray(b) && (b = {buttons: b}); this.c = d.extend(!0, {}, l.defaults, b)
    ...
    add: function (a, b) { var c = this.s.buttons; if (typeof b === 'string') { for (var e = b.split('-'), c = this.s, d = 0, h = e.length - 1; d < h; d++)c = c.buttons[1 * e[d]]; c = c.buttons; b = 1 * e[e.length - 1] } this._expandButton(c, a, !1, b); this._draw(); return this },
...
c; b++) this.remove(a[b].node); this.dom.container.remove(); a = this.s.dt.settings()[0]; b = 0; for (c = a.length; b < c; b++) if (a.inst === this) { a.splice(b, 1); break } return this
    ...
    remove: function (a) { var b = this._nodeToButton(a), c = this._nodeToHost(a), e = this.s.dt; if (b.buttons.length) for (var g = b.buttons.length - 1; g >= 0; g--) this.remove(b.buttons[g].node); b.conf.destroy && b.conf.destroy.call(e.button(a), e, d(a), b.conf); this._removeKey(b.conf); d(b.node).remove(); a = d.inArray(b, c); c.splice(a, 1); return this },
    text: function (a, b) {
      var c = this._nodeToButton(a), e = this.c.dom.collection.buttonLiner, e = c.inCollection && e && e.tag ? e.tag : this.c.dom.buttonLiner.tag,
        g = this.s.dt, h = d(c.node), f = function (a) { return typeof a === 'function' ? a(g, h, c.conf) : a }; if (b === m) return f(c.conf.text); c.conf.text = b; e ? h.children(e).html(f(b)) : h.html(f(b)); return this
    },
...
o.activeElement === o.body) { var c = String.fromCharCode(b.keyCode).toLowerCase(); a.s.listenKeys.toLowerCase().indexOf(c) !== -1 && a._keypress(c, b) }
    ...
    _draw: function (a, b) { a || (a = this.dom.container, b = this.s.buttons); a.children().detach(); for (var c = 0, e = b.length; c < e; c++)a.append(b[c].inserter), a.append(' '), b[c].buttons && b[c].buttons.length && this._draw(b[c].collection, b[c].buttons) },
...
this.s.dt, h = 0, b = !d.isArray(b) ? [b] : b, f = 0, q = b.length; f < q; f++) { var j = this._resolveExtends(b[f]); if (j) if (d.isArray(j)) this._expandButton(a, j, c, e); else { var p = this._buildButton(j, c); if (p) { e !== m ? (a.splice(e, 0, p), e++) : a.push(p); if (p.conf.buttons) { var u = this.c.dom.collection; p.collection = d('<' + u.tag + '/>').addClass(u.className).attr('role', 'menu'); p.conf._collection = p.collection; this._expandButton(p.buttons, p.conf.buttons, !0, e) }j.init && j.init.call(g.button(p.node), g, d(p.node), j); h++ } } }
    ...
    _buildButton: function (a,
      b) {
      var c = this.c.dom.button, e = this.c.dom.buttonLiner, g = this.c.dom.collection, h = this.s.dt, f = function (b) { return typeof b === 'function' ? b(h, j, a) : b }; b && g.button && (c = g.button); b && g.buttonLiner && (e = g.buttonLiner); if (a.available && !a.available(h, a)) return !1; var q = function (a, b, c, e) { e.action.call(b.button(c), a, b, c, e); d(b.table().node()).triggerHandler('buttons-action.dt', [b.button(c), b, c, e]) }, g = a.tag || c.tag, j = d('<' + g + '/>').addClass(c.className).attr('tabindex', this.s.dt.settings()[0].iTabIndex).attr('aria-controls',
        this.s.dt.table().node().id).on('click.dtb', function (b) { b.preventDefault(); !j.hasClass(c.disabled) && a.action && q(b, h, j, a); j.blur() }).on('keyup.dtb', function (b) { b.keyCode === 13 && !j.hasClass(c.disabled) && a.action && q(b, h, j, a) }); g.toLowerCase() === 'a' && j.attr('href', '#'); g.toLowerCase() === 'button' && j.attr('type', 'button'); e.tag ? (g = d('<' + e.tag + '/>').html(f(a.text)).addClass(e.className), e.tag.toLowerCase() === 'a' && g.attr('href', '#'), j.append(g)) : j.html(f(a.text)); !1 === a.enabled && j.addClass(c.disabled); a.className &&
      ...
      if (!b._buttonsHandled) {
        var c = function (e) {
          for (var g = 0, h = e.length; g < h; g++) {
            var f = e[g].conf, q = e[g].node; if (f.key) {
              if (f.key === a)b._buttonsHandled = !0, d(q).click(); else if (d.isPlainObject(f.key) && f.key.key === a && (!f.key.shiftKey || b.shiftKey)) {
...
b.ctrlKey) if (!f.key.metaKey || b.metaKey)b._buttonsHandled = !0, d(q).click()
    ...
    _removeKey: function (a) { if (a.key) { var b = d.isPlainObject(a.key) ? a.key.key : a.key, a = this.s.listenKeys.split(''), b = d.inArray(b, a); a.splice(b, 1); this.s.listenKeys = a.join('') } },
...
'string') { if (!k[c]) throw 'Unknown button type: ' + c; c = k[c] }e++; if (e > 30) throw 'Buttons: Too many iterations'
...
0, q = a.length; f < q; f++)g(a[f])
      } else typeof a === 'string' ? a.indexOf(',') !== -1 ? g(a.split(',')) : (a = d.inArray(d.trim(a), e), a !== -1 && c.push(b[a].inst)) : typeof a === 'number' && c.push(b[a].inst)
    }; g(a); return c
...
d) { f = 0; for (h = a.length; f < h; f++)g(a[f], b) } else if (a === null || a === m || a === '*') { f = 0; for (h = i.length; f < h; f++)c.push({inst: b, node: i[f].node}) } else if (typeof a === 'number')c.push({inst: b, node: b.s.buttons[a].node}); else if (typeof a === 'string') {
          if (a.indexOf(',') !== -1) { i = a.split(','); f = 0; for (h = i.length; f < h; f++)g(d.trim(i[f]), b) } else if (a.match(/^\d+(\-\d+)*$/))f = d.map(i, function (a) { return a.idx }), c.push({inst: b, node: i[d.inArray(a, f)].node}); else if (a.indexOf(':name') !== -1) {
...
i.length; f < h; f++)i[f].name === k && c.push({inst: b, node: i[f].node})
    ...
    dom: {container: {tag: 'div', className: 'dt-buttons'},
      ...
      button: {tag: 'button',
        ...
        active: 'active',
        disabled: 'disabled'},
      buttonLiner: {tag: 'span', className: ''}}}; l.version = '1.5.2'; d.extend(k, {collection: {text: function (a) { return a.i18n('buttons.collection', 'Collection') },
    ...
    action: function (a, b, c, e) {
      var g = d(c).parents('div.dt-button-collection'), a = c.position(), h = d(b.table().container()), f = !1, i = c; g.length && (f = d('.dt-button-collection').position(), i = g, d('body').trigger('click.dtb-collection')); i.parents('body')[0] !== o.body && (i = o.body.lastChild); e._collection.addClass(e.collectionLayout).css('display',
        'none').insertAfter(i).fadeIn(e.fade); g = e._collection.css('position'); f && g === 'absolute' ? e._collection.css({top: f.top, left: f.left}) : g === 'absolute' ? (e._collection.css({top: a.top + c.outerHeight(), left: a.left}), f = h.offset().top + h.height(), f = a.top + c.outerHeight() + e._collection.outerHeight() - f, g = a.top - e._collection.outerHeight(), g = h.offset().top - g, (f > g || e.dropup) && e._collection.css('top', a.top - e._collection.outerHeight() - 5), f = a.left + e._collection.outerWidth(), h = h.offset().left + h.width(), f > h && e._collection.css('left',
  ...
  pageLength: function (a) {
    var a = a.settings()[0].aLengthMenu, b = d.isArray(a[0]) ? a[0] : a, c = d.isArray(a[0]) ? a[1] : a, e = function (a) { return a.i18n('buttons.pageLength', {'-1': 'Show all rows', _: 'Show %d rows'}, a.page.len()) }; return {extend: 'collection',
      text: e,
...
a; return c
    ...
    var e = this; if (!1 === a) return d('#datatables_buttons_info').fadeOut(function () { d(this).remove() }), clearTimeout(r), r = null, this; r && clearTimeout(r); d('#datatables_buttons_info').length && d('#datatables_buttons_info').remove(); d('<div id="datatables_buttons_info" class="dt-button-info"/>').html(a ? '<h2>' + a + '</h2>' : '').append(d('<div/>')[typeof b === 'string' ? 'html'
      : 'append'](b)).css('display', 'none').appendTo('body').fadeIn(); c !== m && c !== 0 && (r = setTimeout(function () { e.buttons.info(!1) }, c)); return this
  }); i.Api.register('buttons.exportData()', function (a) {
    if (this.context.length) {
      var b = new i.Api(this.context[0]), c = d.extend(!0, {}, {rows: null,
          columns: '',
          ...
          customizeData: null}, a), e = function (a) { if (typeof a !== 'string') return a; a = a.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, ''); a = a.replace(/<!\-\-.*?\-\->/g, ''); c.stripHtml && (a = a.replace(/<[^>]*>/g, '')); c.trim && (a = a.replace(/^\s+|\s+$/g, '')); c.stripNewlines && (a = a.replace(/\n/g, ' ')); c.decodeEntities && (v.innerHTML = a, a = v.value); return a }, a = b.columns(c.columns).indexes().map(function (a) { var d = b.column(a).header(); return c.format.header(d.innerHTML, a, d) }).toArray(), g = b.table().footer() ? b.columns(c.columns).indexes().map(function (a) {
          var d =
b.column(a).footer(); return c.format.footer(d ? d.innerHTML : '', a, d)
        }).toArray() : null, h = d.extend({}, c.modifier); b.select && typeof b.select.info === 'function' && h.selected === m && b.rows(c.rows, d.extend({selected: !0}, h)).any() && d.extend(h, {selected: !0}); for (var h = b.rows(c.rows, h).indexes().toArray(), f = b.cells(h, c.columns), h = f.render(c.orthogonal).toArray(), f = f.nodes().toArray(), k = a.length, j = [], l = 0, n = 0, o = k > 0 ? h.length / k : 0; n < o; n++) { for (var r = [k], s = 0; s < k; s++)r[s] = c.format.body(h[l], n, s, f[l]), l++; j[n] = r }a = {header: a,
        footer: g,
        body: j}; c.customizeData && c.customizeData(a); return a
    }
  }); i.Api.register('buttons.exportInfo()', function (a) {
    a || (a = {}); var b; var c = a; b = c.filename === '*' && c.title !== '*' && c.title !== m && c.title !== null && c.title !== '' ? c.title : c.filename; typeof b === 'function' && (b = b()); b === m || b === null ? b = null : (b.indexOf('*') !== -1 && (b = d.trim(b.replace('*', d('head > title').text()))), b = b.replace(/[^a-zA-Z0-9_\u00A1-\uFFFF\.,\-_ !\(\)]/g, ''), (c = t(c.extension)) || (c = ''), b += c); c = t(a.title); c = c === null ? null : c.indexOf('*') !== -1
  ...
  }); var t = function (a) { return a === null || a === m ? null : typeof a === 'function' ? a() : a }, w = function (a, b, c) { b = t(b); if (b === null) return null; a = d('caption', a.table().container()).eq(0); return b === '*' ? a.css('caption-side') !== c ? null : a.length ? a.text() : '' : b }, v = d('<textarea/>')[0]; d.fn.dataTable.Buttons = l; d.fn.DataTable.Buttons = l; d(o).on('init.dt plugin-init.dt',
    function (a, b) { if (a.namespace === 'dt') { var c = b.oInit.buttons || i.defaults.buttons; c && !b._buttons && (new l(b, c)).container() } }); i.ext.feature.push({fnInit: function (a) { var a = new i.Api(a), b = a.init().buttons || i.defaults.buttons; return (new l(a, b)).container() }, cFeature: 'B'}); return l
});

(function (g) { typeof define === 'function' && define.amd ? define(['jquery', 'datatables.net', 'datatables.net-buttons'], function (d) { return g(d, window, document) }) : typeof exports === 'object' ? module.exports = function (d, e) { d || (d = window); if (!e || !e.fn.dataTable)e = require('datatables.net')(d, e).$; e.fn.dataTable.Buttons || require('datatables.net-buttons')(d, e); return g(e, d, d.document) } : g(jQuery, window, document) })(function (g, d, e, h) {
  d = g.fn.dataTable; g.extend(d.ext.buttons, {colvis: function (b, a) {
  ...
  columnVisibility: {columns: h,
    text: function (b, a, c) { return c._columnText(b, c) },
    className: 'buttons-columnVisibility',
    action: function (b, a, c, f) { b = a.columns(f.columns); a = b.visible(); b.visible(f.visibility !== h ? f.visibility : !(a.length && a[0])) },
    init: function (b, a, c) {
...
c.namespace, function (a, d, e) { b.columns(c.columns).count() === 1 && (typeof c.columns === 'number' && (c.columns = e.mapping[c.columns]), a = b.column(c.columns), f.text(c._columnText(b, c)), f.active(a.visible())) }); this.active(b.column(c.columns).visible())
    },
...
26) - 1
    ...
    w === s && (w = y.serializeToString(i.parseXML(B['xl/worksheets/sheet1.xml'])).indexOf('xmlns:r') === -1); i.each(b, function (d, c) {
      if (i.isPlainObject(c)) { var b = a.folder(d); A(b, c) } else {
        if (w) {
          var b = c.childNodes[0], g, e, n = []; for (g = b.attributes.length - 1; g >= 0; g--) { e = b.attributes[g].nodeName; var h = b.attributes[g].nodeValue; e.indexOf(':') !== -1 && (n.push({name: e, value: h}), b.removeAttribute(e)) }g = 0; for (e = n.length; g < e; g++) {
            h = c.createAttribute(n[g].name.replace(':', '_dt_b_namespace_token_')),
            h.value = n[g].value, b.setAttributeNode(h)
          }
        }b = y.serializeToString(c); w && (b.indexOf('<?xml') === -1 && (b = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' + b), b = b.replace(/_dt_b_namespace_token_/g, ':')); b = b.replace(/<([^<>]*?) xmlns=""([^<>]*?)>/g, '<$1 $2>'); a.file(d, b)
      }
    })
  } function q (a, b, d) { var c = a.createElement(b); d && (d.attr && i(c).attr(d.attr), d.children && i.each(d.children, function (a, b) { c.appendChild(b) }), d.text !== null && d.text !== s && c.appendChild(a.createTextNode(d.text))); return c } function K (a, b) {
    var d =
a.header[b].length, c; a.footer && a.footer[b].length > d && (d = a.footer[b].length); for (var f = 0, g = a.body.length; f < g; f++) if (c = a.body[f][b], c = c !== null && c !== s ? c.toString() : '', c.indexOf('\n') !== -1 ? (c = c.split('\n'), c.sort(function (a, c) { return c.length - a.length }), c = c[0].length) : c = c.length, c > d && (d = c), d > 40) return 52; d *= 1.3; return d > 6 ? d : 6
  } var o = i.fn.dataTable, t; var h = typeof self !== 'undefined' && self || typeof j !== 'undefined' && j || this.content; if (typeof h === 'undefined' || typeof navigator !== 'undefined' && /MSIE [1-9]\./.test(navigator.userAgent)) {
    t =
void 0
  } else {
    var x = h.document.createElementNS('http://www.w3.org/1999/xhtml', 'a'), L = 'download' in x, M = /constructor/i.test(h.HTMLElement) || h.safari, C = /CriOS\/[\d]+/.test(navigator.userAgent), N = function (a) { (h.setImmediate || h.setTimeout)(function () { throw a }, 0) }, D = function (a) { setTimeout(function () { typeof a === 'string' ? (h.URL || h.webkitURL || h).revokeObjectURL(a) : a.remove() }, 4E4) }, E = function (a) {
        return /^\s*(?:text\/\S*|application\/xml|\S*\/\S*\+xml)\s*;.*charset\s*=\s*utf-8/i.test(a.type) ? new Blob([String.fromCharCode(65279),
          a], {type: a.type}) : a
      }, F = function (a, b, d) {
        d || (a = E(a)); var c = this, d = a.type === 'application/octet-stream', f, g = function () { for (var a = ['writestart', 'progress', 'write', 'writeend'], a = [].concat(a), b = a.length; b--;) { var d = c['on' + a[b]]; if (typeof d === 'function') try { d.call(c, c) } catch (e) { N(e) } } }; c.readyState = c.INIT; if (L)f = (h.URL || h.webkitURL || h).createObjectURL(a), setTimeout(function () { x.href = f; x.download = b; var a = new MouseEvent('click'); x.dispatchEvent(a); g(); D(f); c.readyState = c.DONE }); else if ((C || d && M) && h.FileReader) {
          var e =
new FileReader(); e.onloadend = function () { var a = C ? e.result : e.result.replace(/^data:[^;]*;/, 'data:attachment/file;'); h.open(a, '_blank') || (h.location.href = a); c.readyState = c.DONE; g() }; e.readAsDataURL(a); c.readyState = c.INIT
      ...
      }, m = F.prototype; typeof navigator !== 'undefined' && navigator.msSaveOrOpenBlob ? t = function (a, b, d) {
      ...
      for (var d = G(b), c = a.buttons.exportData(b.exportOptions), f = b.fieldBoundary,
        g = b.fieldSeparator, e = RegExp(f, 'g'), n = b.escapeChar !== s ? b.escapeChar : '\\', i = function (a) { for (var c = '', b = 0, d = a.length; b < d; b++)b > 0 && (c += g), c += f ? f + ('' + a[b]).replace(e, n + f) + f : a[b]; return c }, h = b.header ? i(c.header) + d : '', j = b.footer && c.footer ? d + i(c.footer) : '', k = [], l = 0, m = c.body.length; l < m; l++)k.push(i(c.body[l])); return {str: h + k.join(d) + j, rows: k.length}
    }, I = function () {
      if (!(navigator.userAgent.indexOf('Safari') !== -1 && navigator.userAgent.indexOf('Chrome') === -1 && navigator.userAgent.indexOf('Opera') === -1)) return !1
      var a = navigator.userAgent.match(/AppleWebKit\/(\d+\.\d+)/); return a && a.length > 1 && 1 * a[1] < 603.1 ? !0 : !1
    }; try { var y = new XMLSerializer(), w } catch (P) {} var B = {'_rels/.rels': '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>',
      'xl/_rels/workbook.xml.rels': '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>',
      '[Content_Types].xml': '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="xml" ContentType="application/xml" /><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml" /><Default Extension="jpeg" ContentType="image/jpeg" /><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml" /><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml" /><Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml" /></Types>',
      'xl/workbook.xml': '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><fileVersion appName="xl" lastEdited="5" lowestEdited="5" rupBuild="24816"/><workbookPr showInkAnnotation="0" autoCompressPictures="0"/><bookViews><workbookView xWindow="0" yWindow="0" windowWidth="25600" windowHeight="19020" tabRatio="500"/></bookViews><sheets><sheet name="" sheetId="1" r:id="rId1"/></sheets></workbook>',
      'xl/worksheets/sheet1.xml': '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" mc:Ignorable="x14ac" xmlns:x14ac="http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac"><sheetData/><mergeCells count="0"/></worksheet>',
    ...
    J = [{match: /^\-?\d+\.\d%$/, style: 60, fmt: function (a) { return a / 100 }}, {match: /^\-?\d+\.?\d*%$/, style: 56, fmt: function (a) { return a / 100 }}, {match: /^\-?\$[\d,]+.?\d*$/, style: 57}, {match: /^\-?£[\d,]+.?\d*$/, style: 58}, {match: /^\-?€[\d,]+.?\d*$/, style: 59}, {match: /^\-?\d+$/, style: 65}, {match: /^\-?\d+\.\d{2}$/, style: 66}, {match: /^\([\d,]+\)$/, style: 61, fmt: function (a) { return -1 * a.replace(/[\(\)]/g, '') }}, {match: /^\([\d,]+\.\d{2}\)$/, style: 62, fmt: function (a) { return -1 * a.replace(/[\(\)]/g, '') }}, {match: /^\-?[\d,]+$/, style: 63},
      {match: /^\-?[\d,]+\.\d{2}$/, style: 64}]; o.ext.buttons.copyHtml5 = {className: 'buttons-copy buttons-html5',
    text: function (a) { return a.i18n('buttons.copy', 'Copy') },
    action: function (a, b, d, c) {
...
'</span>').append(d); b.buttons.info(b.i18n('buttons.copyTitle', 'Copy to clipboard'), h, 0); c[0].focus(); c[0].select(); var m = i(h).closest('.dt-button-info'), k = function () { m.off('click.buttons-copy'); i(l).off('.buttons-copy'); b.buttons.info(!1) }; m.on('click.buttons-copy', k); i(l).on('keydown.buttons-copy', function (a) { a.keyCode === 27 && (k(), f.processing(!1)) }).on('copy.buttons-copy cut.buttons-copy', function () { k(); f.processing(!1) })
    ...
    title: '*',
    messageTop: '*',
    messageBottom: '*'}; o.ext.buttons.csvHtml5 = {bom: !1,
    className: 'buttons-csv buttons-html5',
    available: function () { return j.FileReader !== s && j.Blob },
    text: function (a) { return a.i18n('buttons.csv', 'CSV') },
    action: function (a, b, d, c) { this.processing(!0); var a = H(b, c).str, d = b.buttons.exportInfo(c), f = c.charset; c.customize && (a = c.customize(a, c, b)); !1 !== f ? (f || (f = l.characterSet || l.charset), f && (f = ';charset=' + f)) : f = ''; c.bom && (a = '﻿' + a); t(new Blob([a], {type: 'text/csv' + f}), d.filename, !0); this.processing(!1) },
    filename: '*',
    extension: '.csv',
    exportOptions: {},
    fieldSeparator: ',',
    ...
    charset: null,
    header: !0,
    footer: !1}; o.ext.buttons.excelHtml5 = {className: 'buttons-excel buttons-html5',
      ...
      this.processing(!0); var f = this, g = 0, a = function (a) { return i.parseXML(B[a]) }, e = a('xl/worksheets/sheet1.xml'), h = e.getElementsByTagName('sheetData')[0],
        a = {_rels: {'.rels': a('_rels/.rels')}, xl: {_rels: {'workbook.xml.rels': a('xl/_rels/workbook.xml.rels')}, 'workbook.xml': a('xl/workbook.xml'), 'styles.xml': a('xl/styles.xml'), worksheets: {'sheet1.xml': e}}, '[Content_Types].xml': a('[Content_Types].xml')}, d = b.buttons.exportData(c.exportOptions), l, m, r = function (a) {
          l = g + 1; m = q(e, 'row', {attr: {r: l}}); for (var b = 0, d = a.length; b < d; b++) {
            var f = z(b) + '' + l, j = null; if (a[b] === null || a[b] === s || a[b] === '') if (!0 === c.createEmptyCells)a[b] = ''; else continue; var k = a[b]; a[b] = i.trim(a[b])
            for (var o = 0, r = J.length; o < r; o++) { var p = J[o]; if (a[b].match && !a[b].match(/^0\d+/) && a[b].match(p.match)) { j = a[b].replace(/[^\d\.\-]/g, ''); p.fmt && (j = p.fmt(j)); j = q(e, 'c', {attr: {r: f, s: p.style}, children: [q(e, 'v', {text: j})]}); break } }j || (typeof a[b] === 'number' || a[b].match && a[b].match(/^-?\d+(\.\d+)?$/) && !a[b].match(/^0\d+/) ? j = q(e, 'c', {attr: {t: 'n', r: f}, children: [q(e, 'v', {text: a[b]})]}) : (k = !k.replace ? k : k.replace(/[\x00-\x09\x0B\x0C\x0E-\x1F\x7F-\x9F]/g, ''), j = q(e, 'c', {attr: {t: 'inlineStr', r: f},
              children: {row: q(e,
                'is', {children: {row: q(e, 't', {text: k, attr: {'xml:space': 'preserve'}})}})}}))); m.appendChild(j)
          }h.appendChild(m); g++
    ...
    messageTop: '*',
    messageBottom: '*',
    createEmptyCells: !1}; o.ext.buttons.pdfHtml5 = {className: 'buttons-pdf buttons-html5',
    available: function () {
      return j.FileReader !==
s && (u || j.pdfMake)
    },
    text: function (a) { return a.i18n('buttons.pdf', 'PDF') },
    action: function (a, b, d, c) {
      this.processing(!0); var d = b.buttons.exportData(c.exportOptions), a = b.buttons.exportInfo(c), f = []; c.header && f.push(i.map(d.header, function (a) { return {text: typeof a === 'string' ? a : a + '', style: 'tableHeader'} })); for (var g = 0, e = d.body.length; g < e; g++)f.push(i.map(d.body[g], function (a) { return {text: typeof a === 'string' ? a : a + '', style: g % 2 ? 'tableBodyEven' : 'tableBodyOdd'} })); c.footer && d.footer && f.push(i.map(d.footer, function (a) {
        return {text: typeof a ===
'string' ? a : a + '',
        style: 'tableFooter'}
        ...
        style: 'message',
        margin: [0, 0, 0, 12]}); a.messageBottom && d.content.push({text: a.messageBottom, style: 'message', margin: [0, 0, 0, 12]}); a.title && d.content.unshift({text: a.title, style: 'title', margin: [0, 0, 0, 12]}); c.customize && c.customize(d, c, b); b = (u || j.pdfMake).createPdf(d); c.download === 'open' && !I() ? b.open() : b.download(a.filename); this.processing(!1)
    },
    title: '*',
    filename: '*',
    extension: '.pdf',
...
(function (c) { typeof define === 'function' && define.amd ? define(['jquery', 'datatables.net', 'datatables.net-buttons'], function (f) { return c(f, window, document) }) : typeof exports === 'object' ? module.exports = function (f, b) { f || (f = window); if (!b || !b.fn.dataTable)b = require('datatables.net')(f, b).$; b.fn.dataTable.Buttons || require('datatables.net-buttons')(f, b); return c(b, f, f.document) } : c(jQuery, window, document) })(function (c, f, b, n) {
  var i = c.fn.dataTable, e = b.createElement('a'), m = function (a) {
    e.href = a; a = e.host; a.indexOf('/') === -1 &&
e.pathname.indexOf('/') !== 0 && (a += '/'); return e.protocol + '//' + a + e.pathname + e.search
    ...
    action: function (a, b, e, h) {
      var a = b.buttons.exportData(c.extend({decodeEntities: !1}, h.exportOptions)), e = b.buttons.exportInfo(h), i = c.map(b.settings()[0].aoColumns, function (b) { return b.sClass }), k = function (b, a) {
          for (var d = '<tr>', c = 0, e = b.length; c < e; c++) {
            d += '<' + a + ' ' + (i[c] ? 'class="' + i[c] + '"' : '') + '>' + (b[c] === null || b[c] === n
              ? '' : b[c]) + '</' + a + '>'
          } return d + '</tr>'
        }, d = '<table class="' + b.table().node().className + '">'; h.header && (d += '<thead>' + k(a.header, 'th') + '</thead>'); for (var d = d + '<tbody>', l = 0, o = a.body.length; l < o; l++)d += k(a.body[l], 'td'); d += '</tbody>'; h.footer && a.footer && (d += '<tfoot>' + k(a.footer, 'th') + '</tfoot>'); var d = d + '</table>', g = f.open('', ''); g.document.close(); var j = '<title>' + e.title + '</title>'; c('style, link').each(function () { var b = j, a = c(this).clone()[0]; a.nodeName.toLowerCase() === 'link' && (a.href = m(a.href)); j = b + a.outerHTML })
