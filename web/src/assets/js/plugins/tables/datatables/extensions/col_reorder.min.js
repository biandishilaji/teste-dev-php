...
*/
(function (e) { typeof define === 'function' && define.amd ? define(['jquery', 'datatables.net'], function (o) { return e(o, window, document) }) : typeof exports === 'object' ? module.exports = function (o, l) { o || (o = window); if (!l || !l.fn.dataTable)l = require('datatables.net')(o, l).$; return e(l, o, o.document) } : e(jQuery, window, document) })(function (e, o, l, r) {
...
d; e++)a.childNodes[e].nodeType == 1 && f.push(a.childNodes[e]); b = f[b]; c !== null ? a.insertBefore(b, f[c]) : a.appendChild(b)
  } var t = e.fn.dataTable; e.fn.dataTableExt.oApi.fnColReorder = function (a, b, c, f, g) {
    var d, h, j, m, i, l = a.aoColumns.length, k; i = function (a, b, c) { if (a[b] && typeof a[b] !== 'function') { var d = a[b].split('.'), f = d.shift(); isNaN(1 * f) || (a[b] = c[1 * f] + '.' + d.join('.')) } }; if (b != c) {
      if (b < 0 || b >= l) this.oApi._fnLog(a, 1, "ColReorder 'from' index is out of bounds: " + b); else if (c < 0 || c >= l) {
        this.oApi._fnLog(a, 1, "ColReorder 'to' index is out of bounds: " +
c)
      } else {
        j = []; d = 0; for (h = l; d < h; d++)j[d] = d; p(j, b, c); var n = q(j); d = 0; for (h = a.aaSorting.length; d < h; d++)a.aaSorting[d][0] = n[a.aaSorting[d][0]]; if (a.aaSortingFixed !== null) { d = 0; for (h = a.aaSortingFixed.length; d < h; d++)a.aaSortingFixed[d][0] = n[a.aaSortingFixed[d][0]] }d = 0; for (h = l; d < h; d++) { k = a.aoColumns[d]; j = 0; for (m = k.aDataSort.length; j < m; j++)k.aDataSort[j] = n[k.aDataSort[j]]; k.idx = n[k.idx] }e.each(a.aLastSort, function (b, c) { a.aLastSort[b].src = n[c.src] }); d = 0; for (h = l; d < h; d++) {
          k = a.aoColumns[d], typeof k.mData === 'number'
          ...
          i = this.oApi._fnColumnIndexToVisible(a, b); m = null; for (d = c < b ? c : c + 1; m === null && d < l;)m = this.oApi._fnColumnIndexToVisible(a, d), d++; j = a.nTHead.getElementsByTagName('tr'); d = 0; for (h = j.length; d < h; d++)s(j[d], i, m); if (a.nTFoot !== null) { j = a.nTFoot.getElementsByTagName('tr'); d = 0; for (h = j.length; d < h; d++)s(j[d], i, m) }d = 0; for (h = a.aoData.length; d < h; d++) {
            a.aoData[d].nTr !== null &&
s(a.aoData[d].nTr, i, m)
          }
        }p(a.aoColumns, b, c); d = 0; for (h = l; d < h; d++)a.oApi._fnColumnOptions(a, d, {}); p(a.aoPreSearchCols, b, c); d = 0; for (h = a.aoData.length; d < h; d++) { m = a.aoData[d]; if (k = m.anCells) { p(k, b, c); j = 0; for (i = k.length; j < i; j++)k[j] && k[j]._DT_CellIndex && (k[j]._DT_CellIndex.column = j) }m.src !== 'dom' && e.isArray(m._aData) && p(m._aData, b, c) }d = 0; for (h = a.aoHeader.length; d < h; d++)p(a.aoHeader[d], b, c); if (a.aoFooter !== null) { d = 0; for (h = a.aoFooter.length; d < h; d++)p(a.aoFooter[d], b, c) }(g || g === r) && e.fn.dataTable.Api(a).rows().invalidate()
    ...
    var c = (new e.fn.dataTable.Api(a)).settings()[0]; if (c._colReorder) return c._colReorder; !0 === b && (b = {}); var f = e.fn.dataTable.camelToHungarian; f && (f(i.defaults, i.defaults, !0), f(i.defaults, b || {})); this.s = {dt: null,
      enable: null,
      ...
      aoTargets: []}; this.dom = {drag: null, pointer: null}; this.s.enable = this.s.init.bEnable; this.s.dt = c; this.s.dt._colReorder = this; this._fnConstruct(); return this
  }; e.extend(i.prototype, {fnEnable: function (a) { if (!1 === a) return fnDisable(); this.s.enable = !0 },
    ...
    fnOrder: function (a, b) { var c = [], f, g, d = this.s.dt.aoColumns; if (a === r) { f = 0; for (g = d.length; f < g; f++)c.push(d[f]._ColReorder_iOrigCol); return c } if (b) { d = this.fnOrder(); f = 0; for (g = a.length; f < g; f++)c.push(e.inArray(a[f], d)); a = c } this._fnOrderColumns(q(a)); return this },
    fnTranspose: function (a, b) { b || (b = 'toCurrent'); var c = this.fnOrder(), f = this.s.dt.aoColumns; return b === 'toCurrent' ? !e.isArray(a) ? e.inArray(a, c) : e.map(a, function (a) { return e.inArray(a, c) }) : !e.isArray(a) ? f[a]._ColReorder_iOrigCol : e.map(a, function (a) { return f[a]._ColReorder_iOrigCol }) },
    _fnConstruct: function () {
      ...
      } this.s.dt.oApi._fnCallbackReg(this.s.dt, 'aoStateSaveParams', function (b, c) { a._fnStateSave.call(a, c) }, 'ColReorder_State'); var g = null; this.s.init.aiOrder && (g = this.s.init.aiOrder.slice()); this.s.dt.oLoadedState && (typeof this.s.dt.oLoadedState.ColReorder !== 'undefined' && this.s.dt.oLoadedState.ColReorder.length == this.s.dt.aoColumns.length) && (g = this.s.dt.oLoadedState.ColReorder); if (g) {
...
0; b < a.aaSorting.length; b++)a.aaSorting[b][0] = g[a.aaSorting[b][0]]._ColReorder_iOrigCol; var d = e.extend(!0, [], a.aoSearchCols); b = 0; for (c = g.length; b < c; b++)f = g[b]._ColReorder_iOrigCol, a.aoSearchCols[f] = d[b], a.abVisCols[f] = g[b].bVisible, a.ColReorder.push(f)
    ...
    _fnMouseListener: function (a,
      b) { var c = this; e(b).on('mousedown.ColReorder', function (a) { c.s.enable && c._fnMouseDown.call(c, a, b) }).on('touchstart.ColReorder', function (a) { c.s.enable && c._fnMouseDown.call(c, a, b) }) },
...
this._fnCursorPosition(a, 'pageY') - f.top, this.s.mouse.target = this.s.dt.aoColumns[g].nTh, this.s.mouse.targetIndex = g, this.s.mouse.fromIndex = g, this._fnRegions(), e(l).on('mousemove.ColReorder touchmove.ColReorder', function (a) { c._fnMouseMove.call(c, a) }).on('mouseup.ColReorder touchend.ColReorder', function (a) { c._fnMouseUp.call(c, a) }))
    ...
    _fnMouseMove: function (a) {
      if (this.dom.drag === null) {
        if (Math.pow(Math.pow(this._fnCursorPosition(a, 'pageX') - this.s.mouse.startX, 2) + Math.pow(this._fnCursorPosition(a, 'pageY') -
this.s.mouse.startY, 2), 0.5) < 5) return; this._fnCreateDragNode()
    ...
    _fnMouseUp: function () {
      e(l).off('.ColReorder'); this.dom.drag !== null && (this.dom.drag.remove(),
      this.dom.pointer.remove(), this.dom.drag = null, this.dom.pointer = null, this.s.dt.oInstance.fnColReorder(this.s.mouse.fromIndex, this.s.mouse.toIndex, !0), this._fnSetColumnIndexes(), (this.s.dt.oScroll.sX !== '' || this.s.dt.oScroll.sY !== '') && this.s.dt.oInstance.fnAdjustColumnSizing(!1), this.s.dt.oInstance.oApi._fnSaveState(this.s.dt), this.s.reorderCallback !== null && this.s.reorderCallback.call(this))
    },
    _fnRegions: function () {
      var a = this.s.dt.aoColumns; this.s.aoTargets.splice(0, this.s.aoTargets.length); this.s.aoTargets.push({x: e(this.s.dt.nTable).offset().left,
        to: 0}); for (var b = 0, c = this.s.aoTargets[0].x, f = 0, g = a.length; f < g; f++)f != this.s.mouse.fromIndex && b++, a[f].bVisible && a[f].nTh.style.display !== 'none' && (c += e(a[f].nTh).outerWidth(), this.s.aoTargets.push({x: c, to: b})); this.s.fixedRight !== 0 && this.s.aoTargets.splice(this.s.aoTargets.length - this.s.fixedRight); this.s.fixed !== 0 && this.s.aoTargets.splice(0, this.s.fixed)
    },
